...
const SECTIONS: SectionDef[] = [
    ...
    fields: [
      {
        ...
        placeholder: "Mobile Detailing · Racine & Kenosha County, WI",
      },
      {
        key: "headline",
        label: "Main Headline",
      ...
      },
      {
        ...
        placeholder: "Book My Appointment",
      },
      {
        key: "cta_secondary",
        label: "Secondary Link Text",
        placeholder: "See packages & pricing",
      },
      {
        key: "trust_reviews",
        label: "Trust — Reviews",
        placeholder: "5.0 · Google Reviews",
      },
      {
        key: "trust_certified",
    ...
    fields: [
      {
        ...
        placeholder: "Not a Franchise. Not a Car Wash.",
      },
      {
        key: "body",
        label: "Body Text",
        multiline: true,
        placeholder: "We built Forma Auto Spa around one problem...",
      },
      {
        key: "vehicles_detailed",
        label: "Stat — Vehicles",
        placeholder: "150+",
      },
      { key: "years_experience", label: "Stat — Years", placeholder: "3+" },
      {
        ...
        label: "Stat — Satisfaction",
        placeholder: "100%",
      },
      {
        key: "service_areas",
        label: "Stat — Service Areas",
        placeholder: "15+",
      },
    ],
  },
  {
    ...
    fields: [
      ...
      },
      {
        key: "address",
        label: "City / Address",
        placeholder: "Southeast Wisconsin",
      },
      {
        key: "hours_weekday",
        label: "Weekday Hours",
  ...
  },
  {
    ...
    fields: [
      { key: "name", label: "Business Name", placeholder: "Forma Auto Spa" },
      {
        key: "tagline",
        label: "Tagline",
        placeholder: "Professional Mobile Detailing",
...
function SaveIndicator({ state }: { state: SavingState }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-medium transition-all ${
        state === "saving"
...
function SectionFieldRow({
  ...
  return (
    <FieldRow field={field} initialValue={initialValue} onSave={boundSave} />
  );
}
...
function FAQEditor({
  ...
  const faqs =
    indices.length > 0
      ? indices.map(i => ({
          q: contentMap[`item_${i}_q`] ?? "",
          a: contentMap[`item_${i}_a`] ?? "",
        }))
      : [
          {
          ...
          },
          {
            ...
            a: "Most clients aren't. As long as we can access the vehicle, we'll handle it.",
          },
          {
            q: "How long does a detail take?",
            a: "Exterior or interior alone is about 2 hours. Full Showroom Reset is 3–4 hours.",
          },
          {
            q: "Do you service my area?",
            a: "We cover Racine County, Kenosha County, and the greater Milwaukee metro.",
          },
          {
            q: "What if I'm not happy with the result?",
            a: "Tell us and we'll come back and make it right, no charge.",
          },
        ];
      ...
      {open && (
        <div className="px-5 py-4 bg-background border-t border-border space-y-6">
          {faqs.map((faq, i) => (
            <div
              key={i}
              className="space-y-3 pb-5 border-b border-border/60 last:border-0 last:pb-0"
            >
          ...
          <p className="text-xs text-muted-foreground flex items-center gap-1.5">
            ...
            To add/remove FAQ items, contact your developer. Changes save
            automatically.
          </p>
        </div>
      )}
...
function PackagesEditor() {
  ...
  const upsert = trpc.content.upsertPackage.useMutation({
    onSuccess: () => {
      utils.content.getPackages.invalidate();
          ...
          {isLoading ? (
            ...
            <>
              {pkgs?.map(pkg => (
                <div
                  key={pkg.id}
                  className={`p-4 rounded-xl border bg-card ${!pkg.isActive ? "opacity-50" : ""}`}
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {Math.round(pkg.duration / 60)}h
                      </p>
                    </div>
                    <div className="flex gap-1.5 flex-shrink-0">
                      {pkg.isPopular && (
                        <span className="text-[10px] bg-primary/15 text-primary border border-primary/20 px-2 py-0.5 rounded-full font-semibold">
                          Popular
                        </span>
                      )}
                      <button
          ...
          {editPkg && (
            <div className="mt-2 p-5 rounded-xl border-2 border-primary/30 bg-primary/5 space-y-4">
              <p className="text-sm font-bold">
                {editPkg.id ? "Edit Package" : "New Package"}
              </p>
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2 space-y-1">
                  ...
                  <input
                    value={editPkg.name}
                    onChange={e =>
                      setEditPkg(p => p && { ...p, name: e.target.value })
                    }
                ...
                <div className="col-span-2 space-y-1">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-3 py-2 rounded-lg border border-border bg-input text-sm resize-none focus:outline-none focus:border-primary/60"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    value={editPkg.price}
                    onChange={e =>
                      setEditPkg(p => p && { ...p, price: e.target.value })
                    }
                    className="w-full px-3 py-2 rounded-lg border border-border bg-input text-sm focus:outline-none focus:border-primary/60"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setEditPkg(
                        p => p && { ...p, duration: Number(e.target.value) }
                      )
                    }
                    className="w-full px-3 py-2 rounded-lg border border-border bg-input text-sm focus:outline-none focus:border-primary/60"
                  />
                ...
                <div className="col-span-2 space-y-1">
                  ...
                  <textarea
                    ...
                    rows={5}
                    placeholder="Signature hand wash&#10;Wheel & tire deep clean&#10;…"
                ...
                <label className="flex items-center gap-2 text-xs cursor-pointer">
                  <input
                    type="checkbox"
                    checked={editPkg.isPopular}
                    onChange={e =>
                      setEditPkg(
                        p => p && { ...p, isPopular: e.target.checked }
                      )
                    }
                ...
                <label className="flex items-center gap-2 text-xs cursor-pointer">
                  <input
                    ...
                    onChange={e =>
                      setEditPkg(p => p && { ...p, isActive: e.target.checked })
                    }
                    className="accent-primary"
                  />
                  Active
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => {
                    if (editPkg) upsert.mutate(editPkg);
                    setEditPkg(null);
                  }}
...
function AddOnsEditor() {
  ...
  const upsert = trpc.content.upsertAddOn.useMutation({
    onSuccess: () => {
      utils.content.getAddOns.invalidate();
      ...
      {open && (
        <div className="px-5 py-4 bg-background border-t border-border space-y-3">
          {isLoading ? (
            ...
            <>
              {addons?.map(ao => (
                <div
                  key={ao.id}
                  className={`flex items-center gap-3 p-3 rounded-xl border bg-card ${!ao.isActive ? "opacity-50" : ""}`}
                >
                  ...
                  <button
                    onClick={() => deleteAddon.mutate({ id: ao.id })}
          ...
          {editAddon && (
            <div className="mt-2 p-5 rounded-xl border-2 border-primary/30 bg-primary/5 space-y-4">
              <p className="text-sm font-bold">
                {editAddon.id ? "Edit Add-On" : "New Add-On"}
              </p>
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2 space-y-1">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setEditAddon(a => a && { ...a, name: e.target.value })
                    }
                    className="w-full px-3 py-2 rounded-lg border border-border bg-input text-sm focus:outline-none focus:border-primary/60"
                  />
                </div>
                <div className="col-span-2 space-y-1">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setEditAddon(
                        a => a && { ...a, description: e.target.value }
                      )
                    }
                    className="w-full px-3 py-2 rounded-lg border border-border bg-input text-sm focus:outline-none focus:border-primary/60"
                  />
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    onChange={e =>
                      setEditAddon(a => a && { ...a, price: e.target.value })
                    }
                    className="w-full px-3 py-2 rounded-lg border border-border bg-input text-sm focus:outline-none focus:border-primary/60"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <input
                    type="number"
                    value={editAddon.duration}
                    onChange={e =>
                      setEditAddon(
                        a => a && { ...a, duration: Number(e.target.value) }
                      )
                    }
                    className="w-full px-3 py-2 rounded-lg border border-border bg-input text-sm focus:outline-none focus:border-primary/60"
                  />
                ...
                <label className="flex items-center gap-2 text-xs cursor-pointer col-span-2">
                  <input
                    type="checkbox"
                    checked={editAddon.isActive}
                    onChange={e =>
                      setEditAddon(
                        a => a && { ...a, isActive: e.target.checked }
                      )
                    }
                    className="accent-primary"
                  />
...
export default function AdminSiteEditor() {
  ...
  const save = useCallback(
    async (section: string, key: string, value: string) => {
      await utils.client.content.upsertSiteContent.mutate({
        section,
        key,
        value,
      });
    ...
    <AdminLayout>
      <div className="flex flex-col h-[calc(100vh-64px)]">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Left panel — editor */}
          <div
            className={`flex flex-col overflow-y-auto border-r border-border transition-all ${previewVisible ? "w-[420px] flex-shrink-0" : "flex-1"}`}
          >
