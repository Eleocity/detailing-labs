...
export default function AdminSchedule() {
  ...
  const BookingPill = ({ booking }: { booking: (typeof bookings)[0] }) => (
    <Link href={`/admin/bookings/${booking.id}`}>
      <div
        className={cn(
          "p-1.5 rounded border text-[11px] cursor-pointer hover:opacity-80 transition-opacity mb-1 leading-tight",
          STATUS_COLORS[booking.status] ?? "bg-muted border-border"
        )}
      >
      ...
      <div className="p-3 sm:p-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-display font-bold min-w-0 truncate max-w-[180px] sm:max-w-none">
              {title}
            </span>
            <button
              onClick={() => navigate(1)}
            ...
            <button
              ...
              className="h-8 px-3 rounded-lg border border-border text-xs font-medium hover:bg-muted transition-colors"
            >
              Today
            </button>
          </div>
            ...
            <div className="flex rounded-lg border border-border overflow-hidden">
              {(["agenda", "week", "month"] as ViewMode[]).map(m => (
                <button
                  ...
                  className={cn(
                    "px-3 h-8 text-xs font-medium capitalize transition-colors border-r border-border last:border-r-0",
                    viewMode === m
          ...
          <div>
            {bookings.length === 0 ? (
              <div className="rounded-xl border border-border bg-card p-12 text-center">
                ...
                <Link href="/book">
                  <button className="mt-4 px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold">
                    Book Now
                  </button>
                </Link>
              </div>
              ...
              <div className="space-y-4">
                {agendaDays.map(day => {
                  ...
                  const dayBookings = (bookingsByDate[key] ?? []).sort(
                    (a, b) =>
                  ...
                  return (
                    <div key={key}>
                      ...
                      <div className={cn("flex items-center gap-3 mb-2")}>
                        <div
                          className={cn(
                            "w-9 h-9 rounded-xl flex flex-col items-center justify-center flex-shrink-0 text-center",
                            isToday
                              ? "bg-primary text-primary-foreground"
                      ...
                      <div className="space-y-2 ml-12">
                        {dayBookings.map(b => (
                          <Link key={b.id} href={`/admin/bookings/${b.id}`}>
                            <div
                              className={cn(
                                "p-3.5 rounded-xl border cursor-pointer hover:opacity-80 transition-opacity",
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                ...
                                <div className="text-right flex-shrink-0">
                                  ...
                                  {b.packageName && (
                                    <div className="text-[11px] opacity-70 mt-0.5 max-w-[100px] truncate text-right">
                                      {b.packageName}
                                    </div>
                                  )}
                                  {b.totalAmount && (
              ...
              {weekDays.map(day => {
                ...
                return (
                  <div
                    key={key}
                    className={cn(
                      "p-1 border-r border-border last:border-r-0 min-h-32",
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="grid grid-cols-7 border-b border-border">
              {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
                <div
                  key={i}
                  className="p-2 text-center text-[10px] font-semibold text-muted-foreground border-r border-border last:border-r-0"
                >
                  {d}
                </div>
              ))}
              ...
              {monthGrid.map((day, i) => {
                if (!day)
                  return (
                    <div
                      key={`e-${i}`}
                      className="border-r border-b border-border last:border-r-0 min-h-14 sm:min-h-20 bg-muted/5"
                    />
                  );
                const key = day.toISOString().split("T")[0];
                ...
                return (
                  <div
                    key={key}
                    className={cn(
                      "p-1 border-r border-b border-border last:border-r-0 min-h-14 sm:min-h-20",
                      isToday && "bg-primary/5"
                    )}
                  >
                    <div
                      ...
                      {dayBookings.slice(0, 3).map(b => (
                        <div
                          ...
                          className={cn(
                            ...
                            STATUS_COLORS[b.status]?.includes("blue")
                              ? "bg-blue-400"
                              : STATUS_COLORS[b.status]?.includes("emerald")
                                ? "bg-emerald-400"
                                : STATUS_COLORS[b.status]?.includes("green")
                                  ? "bg-green-400"
                                  : "bg-primary"
                          )}
                        />
