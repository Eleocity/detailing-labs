  ...
  send: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!customerEmail)
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "No customer email on file for this booking",
        });
      ...
      const paymentUrlMatch = inv.notes?.match(
        /Square payment link: (https:\/\/[^\s\n]+)/
      );
      ...
      const content = invoiceEmailV2({
        ...
        serviceAddress:
          [booking?.serviceAddress, booking?.serviceCity, booking?.serviceState]
            .filter(Boolean)
            .join(", ") || "Your location",
        lineItems,
        ...
        notes:
          inv.notes?.replace(/Square payment link:.*$/m, "").trim() ?? null,
        phone,
      ...
      if (!sent)
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Failed to send email — check SENDGRID_API_KEY",
        });
