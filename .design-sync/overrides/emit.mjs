...
//
// FORKED (see cfg.libOverrides in .design-sync/config.json): Forma Auto Spa
// is a dark-only, single-theme app — client/src/index.css sets
// `body { background: var(--background); color: var(--foreground) }`
// unconditionally (#050505 / #f5f5f5), with no light variant. The bundled
// card template hardcoded `background:#fff` on top of that stylesheet,
// which only overrides `background` — `color` still comes from the app's
// own near-white `body` rule, so any element that inherits body text color
// instead of setting its own (Button's outline/ghost/link variants, Label,
// plain paragraph text, the floor-card fallback) renders near-invisible
// near-white-on-white. Fix: stop forcing a white canvas — let the DS's own
// dark body win, and recolor the grid/floor-card chrome (borders, eyebrow,
// fallback title/body text) for a dark canvas instead of a light one.
...
import { escapeHtml, IIFE_IMPORT_META_DEFINE, readText } from '../../.ds-sync/lib/common.mjs';
import { previewExamples } from '../../.ds-sync/lib/docs.mjs';
...
<html><head><meta charset="utf-8">
  ...
  <style>
    ...
    .ds-cell{border:1px solid #2a2a2a;border-radius:8px;padding:12px;min-width:0;overflow:hidden;transform:translateZ(0)}
    .ds-cell>h4{margin:0 0 8px;font:600 12px system-ui;color:#9a9a9a;text-transform:uppercase;letter-spacing:.04em}
...
<html><head><meta charset="utf-8">
  ...
  <template id="ds-fallback">
    <div data-ds-fallback="" style="border:1px solid #2a2a2a;border-radius:12px;padding:28px 24px;max-width:520px;font-family:system-ui">
      <div data-ds-eyebrow="" style="font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:#9a9a9a"></div>
      <div style="font-size:20px;font-weight:600;color:#f5f5f5;margin-top:6px">${escapeHtml(name)}</div>
      <div style="font-size:12px;color:#9a9a9a;margin-top:14px;line-height:1.5">Preview not yet authored. The component is fully importable — its API is in <code>${escapeHtml(name)}.d.ts</code> and usage in <code>${escapeHtml(name)}.prompt.md</code>.</div>
    </div>
  </template>
  <script src="../../../_vendor/react.js"></script>
  <script src="../../../_vendor/react-dom.js"></script>
